...
(defun hierarchicalize-value (element)
  (if (not (getf element :value))
      ...
      (let* ((params (append (getf element :parent-params)
			     ...
			     (getf element :params)))
	     (newval 0))
	(maxima-remember-noclobber params)
	(setf newval (maxima-eval-to-string (maxima-fix-prefixes (getf element
								       :value))))
	(setf newval (concatenate 'string "'1*" newval "'"))
	(maxima-forget-vars params)
	(rplaf element :value newval))))
...
(defun hierarchicalize-params (child parent-params decl-params body-params)
  (maxima-remember-noclobber parent-params)
  (maxima-remember-noclobber decl-params)
  (maxima-remember-noclobber body-params)
  (maxima-remember-noclobber (getf child :params))
  (let* (
	 ;;the order here is apparently important
	 ;;(msg (princ "evaluating parent params"))
	 (evaluated-parent-params
	  (mapcar #'(lambda (pair)
		      (list (car pair) (concatenate 'string 
						    "'1*"
						    (maxima-eval-to-string 
						     (car pair))
						    "'")))
		  parent-params))
	 ;;(msg (princ "hierarchcializing decl params"))
	 (hierarchical-decl-params
	  (mapcar #'(lambda (pair)
		      (list (car pair) (concatenate 'string 
						    "'1*"
						    (maxima-eval-to-string 
						    ...
						    "'")))
		  decl-params))
	 ;;(msg (princ "hierarchicalizing body params"))
	 (hierarchical-body-params
	  (mapcar #'(lambda (pair)
		      (list (car pair) (concatenate 'string 
						    "'1*"
						    (maxima-eval-to-string 
						    ...
						    "'")))
		  body-params))
	 ;;(msg (princ "hierarchicalizing child params"))
	 (hierarchical-child-params
	  (mapcar #'(lambda (pair)
		      (list (car pair) (concatenate 'string 
						    "'1*"
						    (maxima-eval-to-string 
    ...
    ;;maxima needs to forget this element's variables before moving to the next
    (maxima-forget-vars parent-params)
    (maxima-forget-vars decl-params)
    (maxima-forget-vars body-params)
    (maxima-forget-vars (getf child :params))
    ;;return
    (rplaf child
	   :parent-params evaluated-parent-params
	   :decl-params hierarchical-decl-params
	   :body-params hierarchical-body-params
	   :params hierarchical-child-params)))
...
(defun flat-ast (ast dut no-flattens)
  ...
  (let* (;;find instances within the elements of dut
	 ...
	 ;;collect global params to pass to flattening subroutines
	 (global-params 
	  (getf (find-if #'(lambda (x) (getf x :global-params)) ast)
		:global-params))
	 (side-effect (maxima-remember-noclobber global-params))
	 ;;recursively flatten child elements
	 (flattened-dut-elements
	  (flatten-elements dut-elements '() "" symbol-table no-flattens))
	 ;;substitute flattened elements into dut-decl
	 (flattened-dut-decl
	  (rplaf (copy-tree dut-decl) :elements flattened-dut-elements))
	 (result (nsubstitute flattened-dut-decl dut-decl ast))
	 (msg (maxima-forget-vars global-params)))
    result))
